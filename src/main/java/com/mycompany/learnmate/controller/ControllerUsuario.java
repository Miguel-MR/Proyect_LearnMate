...
public class ControllerUsuario implements Serializable {
    ...
    public List<Personas> listarPersonasRolProfesor(){
        ...
        List<Personas> ps = pfl.findAll();
        for(RolesUsuario u : users){
            for(Personas p : ps){
                if(u.getUsuarioRolId().equals(p.getPersonaId())){
                    per.add(p);
                }
            }
        }
    ...
    public List<SelectItem> listarPP (){
        ...
        try{
            for(Personas p : listarPersonas ){
                listarPer.add(new SelectItem (p.getPersonaId(),p.getPrimerNombre()));
            }
         return listarPer; 
        }        
        catch (Exception e){
